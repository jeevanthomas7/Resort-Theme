...
import { Maximize, Bed, Users, Wind, Wifi, Tv, Coffee, Refrigerator, ShieldCheck, Bath, ChevronLeft, ChevronRight, ArrowRight } from 'lucide-react';
import TestimonialSection from '../components/sections/TestimonialSection';

const images = [
  "/images/standard/s3.jpg",
  "/images/standard/s4.avif",
  "/images/standard/s2.jpg"
];
        ...
        <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-12">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20 items-stretch mb-10">
            ...
            <div className="lg:col-span-7 relative group rounded-2xl overflow-hidden shadow-2xl h-[350px] sm:h-[450px] lg:h-[600px] w-full border border-primary/10">
              <a href={images[currentIdx]} className="glightbox block w-full h-full" data-gallery="standard">
                <img 
                  id="main-room-img" 
                  src={images[currentIdx]} 
                  className="w-full h-full object-cover transition-transform duration-700 hover:scale-105"
                  alt="Standard Himalayan Room"
                />
              </a>
              ...
              <div className="hidden">
                {images.map((img, idx) => (
                  <a key={idx} href={img} className="glightbox" data-gallery="standard"></a>
                ))}
              </div>
            </div>
            ...
            <div className="lg:col-span-5 flex flex-col bg-cream/30 p-8 sm:p-12 rounded-[2.5rem] border border-primary/10">
              <div className="mb-8 font-inter">
                <span className="text-accent text-[11px] font-bold tracking-[0.4em] uppercase block mb-4">Room Details</span>
                <h2 className="font-serif text-4xl sm:text-5xl text-primary leading-tight mb-6 font-bold">Standard Himalayan Room</h2>
                <p className="text-text/70 text-sm sm:text-base leading-relaxed font-sans">
                  Experience the perfect balance of simplicity and luxury. Our Standard Rooms offer a warm, inviting atmosphere with handcrafted Himalayan wood finishes and large windows overlooking the mist-covered valleys.
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-y-6 gap-x-4 border-y border-primary/10 py-8 mb-8 font-inter">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-primary font-bold text-xs uppercase tracking-widest">1200 SQFT</span>
                </div>
                <div className="flex items-center gap-3">
                  <Bed className="w-5 h-5 text-accent" />
                  <span className="text-primary font-bold text-xs uppercase tracking-widest">1 King Bed</span>
                </div>
                <div className="flex items-center gap-3">
                  <Users className="w-5 h-5 text-accent" />
                  <span className="text-primary font-bold text-xs uppercase tracking-widest">2-3 Guests</span>
                </div>
                <div className="flex items-center gap-3">
                  <Wind className="w-5 h-5 text-accent" />
                  <span className="text-primary font-bold text-xs uppercase tracking-widest">A/C Room</span>
                </div>
              </div>
          ...
          <div className="mb-0 py-10">
            <h3 className="font-serif text-3xl text-primary font-bold mb-12 text-center lg:text-left tracking-tight">Room Amenities</h3>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6 sm:gap-8 text-center lg:text-left">
              ...
              <div className="flex flex-col items-center lg:items-start gap-4 group">
                ...
                <span className="text-primary font-bold text-[10px] uppercase tracking-widest font-inter">Smart TV</span>
              </div>
              <div className="flex flex-col items-center lg:items-start gap-4 group">
                <div className="w-12 h-12 rounded-2xl bg-cream border border-primary/5 flex items-center justify-center text-accent group-hover:bg-primary group-hover:text-white transition-all shadow-sm">
                  <Coffee className="w-6 h-6" />
                </div>
                <span className="text-primary font-bold text-[10px] uppercase tracking-widest font-inter">Tea/Coffee</span>
              </div>
              <div className="flex flex-col items-center lg:items-start gap-4 group">
                <div className="w-12 h-12 rounded-2xl bg-cream border border-primary/5 flex items-center justify-center text-accent group-hover:bg-primary group-hover:text-white transition-all shadow-sm">
                  <Refrigerator className="w-6 h-6" />
                </div>
                <span className="text-primary font-bold text-[10px] uppercase tracking-widest font-inter">Mini Bar</span>
              </div>
              <div className="flex flex-col items-center lg:items-start gap-4 group">
                <div className="w-12 h-12 rounded-2xl bg-cream border border-primary/5 flex items-center justify-center text-accent group-hover:bg-primary group-hover:text-white transition-all shadow-sm">
                  <ShieldCheck className="w-6 h-6" />
                </div>
                <span className="text-primary font-bold text-[10px] uppercase tracking-widest font-inter">Safe Box</span>
              </div>
              <div className="flex flex-col items-center lg:items-start gap-4 group">
                <div className="w-12 h-12 rounded-2xl bg-cream border border-primary/5 flex items-center justify-center text-accent group-hover:bg-primary group-hover:text-white transition-all shadow-sm">
                  <Bath className="w-6 h-6" />
                </div>
                <span className="text-primary font-bold text-[10px] uppercase tracking-widest font-inter">Luxury Bath</span>
              </div>
            </div>
          </div>
